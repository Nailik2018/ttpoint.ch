<title>Document</title>
<link rel="stylesheet" href="https://ttpoint.ch/v1/css/navigation.css">
</head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-danger" id="navbar-ttpoint">
    <a class="navbar-brand" href="https://ttpoint.ch"><img src="https://ttpoint.ch/v1/assets/logos/table-tennis1.svg"></a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target=".navbar-collapsed" aria-controls="navbarText" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon" onclick="showMenu()"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarText">
        <ul class="navbar-nav mr-auto">
            <li class="nav-item active">
                <a class="nav-link" href="https://ttpoint.ch/v1/sites/associations/">Verbände<span class="sr-only">(current)</span></a>
                <a class="nav-link" href="https://ttpoint.ch/v1/sites/rankings/">Rankings<span class="sr-only"></span></a>
            </li>
        </ul>
        <span class="navbar-text">
    </span>
    </div>
</nav>
<script type="text/javascript" src="https://ttpoint.ch/v1/js/navigation.js"></script>
